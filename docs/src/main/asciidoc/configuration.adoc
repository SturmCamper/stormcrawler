...
| fetcher.threads.start.delay | 10 | Delay (seconds) before starting fetcher threads.
...
| http.retry.on.connection.failure | true | Retry fetching on connection failure.
